...
const fetchAcceptedCount = async () => {
    ...
    try {
        const response = await fetch(`${adminUrl}/api/competitions/accepted-count`, {
        ...
        cachedAcceptedCount = data.acceptedCount ?? 0;
        cacheTimestamp = now;
        return cachedAcceptedCount;
    } catch {
        return null;
...
export const getDropConfig = () => {
    ...
    const minGladiators = normalizeCount(parseNumber(process.env.DROP_MIN_GLADIATORS, 6));
