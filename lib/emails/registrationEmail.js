...
async function generateRegistrationEmail(registration, dropInfo) {
  try {
    ...
    const userLang = registration.language || 'es';
    ...
    const context = {
      ...
      drop_name_or_number: dropInfo ? dropInfo.name : (userLang === 'en' ? "Next Drop" : "Próximo Drop"),
...
OBJETIVO
Escribir un email de bienvenida/confirmación para un gladiador que acaba de registrarse. Debe sentirse como una proclamación del Coliseo (rol-play), no como email de marketing. Tono: épico, oscuro-divertido, directo, con vibra de arena y gloria. Sin clichés corporativos, sin "estamos emocionados", sin lenguaje de startup.
